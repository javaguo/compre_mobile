		...
		<block v-for="(item,index) in list.data" :key="index">
			<view class="com_list">
				<view class="com_list_row com_list_row_top">
					...
					<view :class="item.fkParentId==null ? 'com_bold' : ''" class="list_text list_type uni-ellipsis">{{item.fkParentId!=null?item.fkParentId:item.expendTypeName}}</view>
					<view class="list_text list_type uni-ellipsis">{{item.fkParentId!=null?item.expendTypeName:""}}</view>
	...
	export default {
		...
		methods: {
			loadList(e, opeType) {
				...
				let reqURL = config.SERVER_URL + "m/expendType/searchData.do";
				uni.request({
					url: reqURL,
					data: {
			...
			save(typeLevel) { // 保存数据
				let id,fkParentId,expendTypeName,orderNum;
				...
				if (typeLevel=="level1"){// 处理一级类型相关字段
					...
					expendTypeName = this.typeNameLevel1;
					...
					expendTypeName = this.typeNameLevel2;
				...
				if ( id!=undefined && id!= null && id != '') { // 编辑
					tempURL = config.SERVER_URL + "m/expendType/update.do";
					reqParam.id = id;
					opeTypeTip = '修改';
				} else { // 添加
					tempURL = config.SERVER_URL + "m/expendType/save.do";
					opeTypeTip = '保存';
				}
				uni.request({
					url: tempURL,
			...
			editData(e, item) { // 编辑数据事件
				...
				let reqURL = config.SERVER_URL + "m/expendType/edit.do";
				uni.request({
					url: reqURL,
					data: {
						loginName: this.loginName,
					...
					success: (res) => {
						...
						if (res.data.success) {
							if (res.data.bean.fkParentId==-1){// 一级类型
								...
								this.typeNameLevel1 = res.data.bean.expendTypeName;
								...
								this.typeNameLevel2 = res.data.bean.expendTypeName;
				...
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) { // 确定删除
							...
							let reqURL = config.SERVER_URL + "m/expendType/delete.do";
							uni.request({
								url: reqURL,
								data: {
									loginName: this.loginName,
			...
			loadType() {
				...
				let reqURL = config.SERVER_URL +
					"m/expendType/loadTreeData.do?fieldMap=value:id,label:expend_type_name,parentId:fk_parent_id&treeRootVal=-1&treeFlag=expendTypeLevel1&resType=map&multiSelect=false";
				uni.request({
					url: reqURL,
					data: {
						loginName: this.loginName,
